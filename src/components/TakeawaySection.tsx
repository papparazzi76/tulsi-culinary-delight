    ...
    <div className="space-y-12">
      ...
      {degustacion.menus.map((menu, index) => (
        <div key={index} className="bg-card border border-border rounded-xl p-6 transition-all duration-300">
          <h3 className="text-3xl font-bold text-accent mb-2 font-playfair">{menu.name}</h3>
          <p className="text-md text-muted-foreground mb-6">{menu.price}</p>
          <div className="grid md:grid-cols-2 gap-6">
            {Object.entries(menu.sections).map(([sectionTitle, items]) => (
              <div key={sectionTitle}>
                <h4 className="text-xl font-semibold text-foreground font-playfair mb-4">{sectionTitle}</h4>
                <ul className="space-y-2">
                  {(items as MenuItem[]).map((item, itemIndex) => (
